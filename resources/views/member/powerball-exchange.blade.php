...
@section("content")
    <div class="content">
        ...
        <form name="exchangeForm" method="post" action="{{route("request-exchange")}}" enctype="multipart/form-data">
            @csrf
        ...
        <table class="logBox table" style="margin-top:5px;">
            ...
            <tr>
                <td>{{$ex["id"]}}</td>
                <td>
                    @if($ex["status"] == 0){{"신청"}}@endif
                    @if($ex["status"] == 1){{"승인"}}@endif
                    @if($ex["status"] == 2){{"거절"}}@endif
                </td>
                <td>
                    {{$ex["bullet"]}}
                </td>
                <td>
                    {{number_format($ex["money"])}}원
                </td>
                <td>
                    {{$ex["updated_at"]}}
                </td>
