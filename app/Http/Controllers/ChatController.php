...
class ChatController extends Controller
{
    ...
    public function roomWait(Request $request){
        ...
        $p = new PbRoom();
        if($rtype == "winRate"){
            $pb_rooms = $p->with("roomandpicture.getUserClass")->where("pb_room.active",1)->get()->toArray();
        }
        if($rtype == "userCnt"){
            $pb_rooms = $p->with("roomandpicture.getUserClass")->where("pb_room.active",1)->orderBy("members","DESC")->get()->toArray();
        }
        if($rtype == "recommend"){
            $pb_rooms = $p->with("roomandpicture.getUserClass")->where("pb_room.active",1)->orderBy("recommend","DESC")->get()->toArray();
        }
        if($rtype == "latest"){
            $pb_rooms = $p->with("roomandpicture.getUserClass")->where("pb_room.active",1)->orderBy("created_at","DESC")->get()->toArray();
        }
        ...
        if($rtype == "favor"){
            ...
            if(empty($favor_list["flist"]))
                ...
                $pb_rooms = $p->with("roomandpicture.getUserClass")
                    ->where("pb_room.active",1)
                    ->whereIn("pb_room.roomIdx",explode(",",$favor_list["flist"]))->orderBy("updated_at","DESC")->get()->toArray();
        }
    ...
    public function createRoom(Request $request){
        ...
        $active_count = PbRoom::where("super",$user->userIdKey)->get()->count();
        if($active_count > 0)
        {
    ...
    public function checkActiveRoom(Request $request){
        ...
        if(!empty($checkd)){
            ...
            if(!empty($active_room)){
                ...
                echo json_encode(array("status"=>0,"msg"=>"개설한 채팅방이 존재하지 않습니다."));
            }
        }
        return;
    }
